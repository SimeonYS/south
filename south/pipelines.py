...
class SouthPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('south.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `south`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM south WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO south (date, title, link, content)"
